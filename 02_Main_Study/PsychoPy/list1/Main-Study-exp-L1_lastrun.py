...
expInfo = {'participant': '', 'E-Mail-Adresse': '', 'Vorname': '', 'Nachname': '', 'Alter': '', 'Links- oder Rechtshänder': '', 'Zweitsprache (1 geringe Kenntnisse - 10 muttersprachlich)': '', '2. Zweitsprache (1 - 10)': '', '3. Zweitsprache (1 - 10)': '', '4. Zweitsprache (1 - 10)': '', '5. Zweitsprache (1 - 10)': ''}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\alexa\\Documents\\02_study\\duesseldorf\\BA_Linguistik\\09_BBA_Bachelorarbeit\\02_daten\\02_Main_Study\\PsychoPy\\list1\\Main-Study-exp-L1_lastrun.py',
...
Explaination2_ScreenClock = core.Clock()
...
Explaination_Screen3Clock = core.Clock()
Explaination3_text = visual.TextStim(win=win, name='Explaination3_text',
    text='',
    font='Open Sans',
    pos=(0, 0), height=0.04, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
    depth=0.0);
Explaination3_key_resp = keyboard.Keyboard()
...
# keep track of which components have finished
Explaination2_ScreenComponents = [Explaination2_text, Explaination2_key_resp]
for thisComponent in Explaination2_ScreenComponents:
    thisComponent.tStart = None
    thisComponent.tStop = None
    thisComponent.tStartRefresh = None
    thisComponent.tStopRefresh = None
...
t = 0
_timeToFirstFrame = win.getFutureFlipTime(clock="now")
Explaination2_ScreenClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
frameN = -1

# -------Run Routine "Explaination2_Screen"-------
while continueRoutine:
    # get current time
    t = Explaination2_ScreenClock.getTime()
    tThisFlip = win.getFutureFlipTime(clock=Explaination2_ScreenClock)
    tThisFlipGlobal = win.getFutureFlipTime(clock=None)
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
...
trials00_Loop = data.TrialHandler(nReps=1.0, method='sequential', 
    ...
    trialList=data.importConditions('Main-Study-stimuli-Examples.xlsx', selection='0:5'),
...
for thisTrials00_Loop in trials00_Loop:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if Pos01_Name_text.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > Pos01_Name_text.tStartRefresh + 2-frameTolerance:
                # keep track of stop time/frame for later
                Pos01_Name_text.tStop = t  # not accounting for scr refresh
                Pos01_Name_text.frameNStop = frameN  # exact frame index
                win.timeOnFlip(Pos01_Name_text, 'tStopRefresh')  # time at next scr refresh
                Pos01_Name_text.setAutoDraw(False)
        ...
        if Pos01_Name_key_resp.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > Pos01_Name_key_resp.tStartRefresh + 2-frameTolerance:
                # keep track of stop time/frame for later
                Pos01_Name_key_resp.tStop = t  # not accounting for scr refresh
                Pos01_Name_key_resp.frameNStop = frameN  # exact frame index
                win.timeOnFlip(Pos01_Name_key_resp, 'tStopRefresh')  # time at next scr refresh
                Pos01_Name_key_resp.status = FINISHED
...
Explaination3_key_resp.keys = []
Explaination3_key_resp.rt = []
_Explaination3_key_resp_allKeys = []
# keep track of which components have finished
Explaination_Screen3Components = [Explaination3_text, Explaination3_key_resp]
for thisComponent in Explaination_Screen3Components:
    thisComponent.tStart = None
    thisComponent.tStop = None
    thisComponent.tStartRefresh = None
    thisComponent.tStopRefresh = None
...
t = 0
_timeToFirstFrame = win.getFutureFlipTime(clock="now")
Explaination_Screen3Clock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
frameN = -1

# -------Run Routine "Explaination_Screen3"-------
while continueRoutine:
    # get current time
    t = Explaination_Screen3Clock.getTime()
    tThisFlip = win.getFutureFlipTime(clock=Explaination_Screen3Clock)
    tThisFlipGlobal = win.getFutureFlipTime(clock=None)
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
    
    # *Explaination3_text* updates
    if Explaination3_text.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        Explaination3_text.frameNStart = frameN  # exact frame index
        Explaination3_text.tStart = t  # local t and not account for scr refresh
        Explaination3_text.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(Explaination3_text, 'tStartRefresh')  # time at next scr refresh
        Explaination3_text.setAutoDraw(True)
    
    # *Explaination3_key_resp* updates
    waitOnFlip = False
    if Explaination3_key_resp.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        Explaination3_key_resp.frameNStart = frameN  # exact frame index
        Explaination3_key_resp.tStart = t  # local t and not account for scr refresh
        Explaination3_key_resp.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(Explaination3_key_resp, 'tStartRefresh')  # time at next scr refresh
        Explaination3_key_resp.status = STARTED
        # keyboard checking is just starting
        waitOnFlip = True
        win.callOnFlip(Explaination3_key_resp.clock.reset)  # t=0 on next screen flip
        win.callOnFlip(Explaination3_key_resp.clearEvents, eventType='keyboard')  # clear events on next screen flip
    if Explaination3_key_resp.status == STARTED and not waitOnFlip:
        theseKeys = Explaination3_key_resp.getKeys(keyList=['space'], waitRelease=False)
        _Explaination3_key_resp_allKeys.extend(theseKeys)
        if len(_Explaination3_key_resp_allKeys):
            Explaination3_key_resp.keys = _Explaination3_key_resp_allKeys[-1].name  # just the last key pressed
            Explaination3_key_resp.rt = _Explaination3_key_resp_allKeys[-1].rt
            # a response ends the routine
            continueRoutine = False
...
for thisComponent in Explaination_Screen3Components:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
thisExp.addData('Explaination3_text.started', Explaination3_text.tStartRefresh)
thisExp.addData('Explaination3_text.stopped', Explaination3_text.tStopRefresh)
# check responses
if Explaination3_key_resp.keys in ['', [], None]:  # No response was made
    Explaination3_key_resp.keys = None
thisExp.addData('Explaination3_key_resp.keys',Explaination3_key_resp.keys)
if Explaination3_key_resp.keys != None:  # we had a response
    thisExp.addData('Explaination3_key_resp.rt', Explaination3_key_resp.rt)
thisExp.addData('Explaination3_key_resp.started', Explaination3_key_resp.tStartRefresh)
thisExp.addData('Explaination3_key_resp.stopped', Explaination3_key_resp.tStopRefresh)
thisExp.nextEntry()
...
trials01_Loop = data.TrialHandler(nReps=1.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('Main-Study-stimuli-L1-B1.xlsx', selection='0:26'),
...
trials02_Loop = data.TrialHandler(nReps=1.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('Main-Study-stimuli-L1-B2.xlsx', selection='0:21'),
...
trials03_Loop = data.TrialHandler(nReps=1.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('Main-Study-stimuli-L1-B3.xlsx', selection='0:21'),
...
for thisTrials03_Loop in trials03_Loop:
    ...
    if Comp_q_key_resp.keys in ['', [], None]:  # No response was made
        Comp_q_key_resp.keys = None
    trials03_Loop.addData('Comp_q_key_resp.keys',Comp_q_key_resp.keys)
    if Comp_q_key_resp.keys != None:  # we had a response
        trials03_Loop.addData('Comp_q_key_resp.rt', Comp_q_key_resp.rt)
    trials03_Loop.addData('Comp_q_key_resp.started', Comp_q_key_resp.tStartRefresh)
    trials03_Loop.addData('Comp_q_key_resp.stopped', Comp_q_key_resp.tStopRefresh)
    trials03_Loop.addData('Comp_q_text.started', Comp_q_text.tStartRefresh)
    trials03_Loop.addData('Comp_q_text.stopped', Comp_q_text.tStopRefresh)
    # the Routine "Comp_q" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
...
trials04_Loop = data.TrialHandler(nReps=1.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('Main-Study-stimuli-L1-B4.xlsx', selection='0:21'),
...
trials05_Loop = data.TrialHandler(nReps=1.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('Main-Study-stimuli-L1-B5.xlsx', selection='0:21'),
...
trials06_Loop = data.TrialHandler(nReps=1.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('Main-Study-stimuli-L1-B6.xlsx', selection='0:21'),
